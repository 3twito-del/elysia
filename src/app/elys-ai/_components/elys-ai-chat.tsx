...
type ElysAiChatProps = {
  compact?: boolean;
  queuedMessage?: QueuedChatMessage;
  onQueuedMessageSent?: (id: number) => void;
};

export function ElysAiChat({
  compact = false,
  queuedMessage,
  onQueuedMessageSent,
}: ElysAiChatProps) {
  const [input, setInput] = useState("");
  const lastQueuedMessageId = useRef<number | null>(null);
  const {
  ...
  useEffect(() => {
    if (!queuedMessage || lastQueuedMessageId.current === queuedMessage.id) {
      return;
    }

    lastQueuedMessageId.current = queuedMessage.id;
    void sendMessage({ text: queuedMessage.text });
    onQueuedMessageSent?.(queuedMessage.id);
  }, [onQueuedMessageSent, queuedMessage, sendMessage]);
