...
| RPT-003 | [M] 🟡 | ייצוא **CSV** (`toCsv` בנוי); **Excel נבנה:** `reportToMatrix` טהור (משותף ל-CSV/Excel) + `report-export.ts` (`buildReportXlsx` עם `exceljs`, RTL, מדדים מספריים) ונתיב הורדה `/api/admin/reports/[id]/export?format=csv|xlsx` (CSV עם BOM לעברית) עם כפתורי Excel/CSV ב-`/admin/reports`. PDF + דוחות מתוזמנים — בהמשך. |
