...
| FIN-AR-001 | [M] 🟡 | AR מלא: חשבוניות לקוח, קבלות, גיול, גבייה, dunning. **נבנה:** AR (חשבוניות/קבלות/גיול) קיים; **Dunning נבנה:** `dunning.ts` (`daysOverdue`/`dunningLevel`/`buildDunningWorklist` טהורים) + `DunningAction` — worklist חשבוניות באיחור לפי רמת הסלמה (1–4) ותיעוד פנייה ב-`/admin/finance`. **שליחת תזכורת באימייל נבנתה:** `buildDunningEmail` (טהור, טון פר-רמה) + `sendDunningReminder` דרך ה-adapter (Brevo) — כפתור "שלח תזכורת" ללקוח עם דוא"ל, מתעד DunningAction. |
...
| HR-003 | [S] 🟡 | **Payroll ישראלי** — מס הכנסה, ביטוח לאומי, בריאות, פנסיה/קה"ש, תלוש, טופס 101/106, דיווח 102/126. **נבנה (בהחלטת בעלים):** `israeli-payroll.ts` (טהור) — מנוע חישוב סטטוטורי מלא: מס הכנסה מדורג (`computeGrossIncomeTax`/`computeIncomeTax`) עם נקודות זיכוי, ביטוח לאומי + מס בריאות דו-שכבתיים (`computeTieredContribution`), ופנסיית עובד → `computeIsraeliPayslip` (פירוק נטו מלא). מחשבון שכר ב-`/admin/performance`. **שיעורים/מדרגות לדוגמה 2024 — לאמת רו"ח**; טפסים 101/106/102/126 וחיווט לריצת ה-GL החודשית — בהמשך (דורש תבניות רשמיות + אימות). |
...
| מזהה | חיוב | דרישה |
|---|---|---|
| AI-001 | [S] 🟡 | **Copilot פר-מודול** — "מה מצב התזרים?", "אילו לקוחות בסיכון?", עם הקשר מלא והרשאות המשתמש. **נבנה:** `admin-copilot.ts` (`buildCopilotContext`/`fallbackAnswer` טהורים) — Q&A ניהולי מעוגן ב-snapshot מדדים חיים (גבייה/FX/רווחיות) דרך `quota-router` הקיים, המלצה-בלבד (לא כותב לספרים, תואם AI-005), עם fallback דטרמיניסטי ללא מפתח AI. מסך ב-`/admin/ai`. הרחבה לכל מודול + tool-calling — בהמשך. |
| AI-002 | [S] 🟡 | **NL-Query** — שאילתה בשפה טבעית מעל הנתונים → SQL/tRPC מאומת + תוצאה מוסברת. **נבנה:** `nl-query.ts` (`buildNlQuerySchemaContext`/`resolveNlQuerySelection` טהורים) — ה-LLM ממפה שאלה ל-datasetKey+ממדים+מדדים מתוך השכבה הסמנטית המוגדרת (structured output מאומת ב-Zod), ואז מריץ את מנוע הדוחות הטהור. **ללא SQL חופשי** — מוגבל למאגרים הקיימים, לכן בטוח. מסך ב-`/admin/reports`, fallback ללא מפתח AI. |
| AI-003 | [S] | **Predictions** — churn, demand, credit risk, next-best-action (מחליף heuristics בחוקים+מודל). |
| AI-004 | [S] 🟡 | **Document-AI** — OCR לחשבוניות ספק/קבלות → טיוטת AP עם 3-way match. **נבנה:** `document-ai.ts` (`extractionToLinesText` טהור) + `DocumentExtraction` — הדבקת טקסט חשבונית → חילוץ מובנה (ספק/מספר/תאריך/שורות) ב-structured output מאומת Zod, נשמר כטיוטה לבדיקה אנושית ב-`/admin/erp`. חילוץ בלבד (לא רושם לספרים). **OCR מתמונה נבנה:** `extractInvoiceFromImage` (Gemini vision — העלאת PNG/JPEG/WEBP/PDF עד 8MB → חילוץ מובנה). **המרה ל-AP נבנתה:** "צור טיוטה" בכל חילוץ — המנהל בוחר ספק תואם וה-חילוץ הופך ל-`createVendorInvoice` (טיוטה, נכנסת ל-3-way הקיים) עם human-in-the-loop. fallback ללא מפתח AI. |
...
| NFR-AVAIL-001 | [M] | יעד זמינות 99.9%; RPO ≤ 5 דק', RTO ≤ 1ש' לנתונים פיננסיים. **דרישה מודגמת, לא מוצהרת:** PITR פעיל בספק ה-Postgres = תנאי השקה; תרגיל שחזור אמיתי (restore drill) עם ראיות ו-runbook = קבלת השקה; migrations הרסניים על טבלאות פיננסיות מחייבים נקודת-שחזור + תוכנית היפוך. ראה `docs/adr/0008-database-durability-pitr-restore-drill.md`. |
...
- **SEC-005 [M]** Audit immutability — נאכף **מתחת לשכבת ה-service** בטריגרים של Postgres (migration ייעודי + בדיקות אינטגרציה שמוכיחות דחייה): חסימת UPDATE/DELETE מלאה על `AuditLog`/`JournalLine`/`InventoryLedger`/`ConsentRecord`/`LoyaltyTransaction`; על `JournalEntry` מותר אך ורק המעבר `POSTED → REVERSED`. מוחרגים במוצהר: `OutboxEvent` (מכונת-מצבים תפעולית) ו-`ItemCostLayer` (טבלת שערוך עבודה — האמת הבלתי-משתנה היא `LandedCost`+GL+`AuditLog`). הפרדת roles (runtime ללא UPDATE/DELETE) — פריט hardening לאחר השקה. ראה `docs/adr/0004-db-level-immutability-triggers.md`.
- **SEC-006 [M]** הפרדת שתי זרימות האימות נשמרת, אך ה-admin הוא control plane: **TOTP חובה לכל admin (כולל bootstrap)** עם recovery codes חד-פעמיים hash-stored; Email-OTP נדחה כגורם שני; sessions אדמין ≤ 12 שעות (נפרד ממדיניות לקוחות); `src/middleware.ts` מגן על `/admin` וכל ה-admin APIs (הגנה בשכבות — לא מחליף הרשאות per-procedure); rate-limit פר-חשבון+IP, lockout מדורג, ואירועי audit לכל login כושל/מוצלח/TOTP/recovery. Passkeys ו-subdomain אדמין — hardening לאחר השקה. ראה `docs/adr/0005-admin-authentication-and-isolation.md`.
