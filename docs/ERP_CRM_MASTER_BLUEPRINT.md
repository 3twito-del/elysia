...
| מזהה | חיוב | דרישה |
|---|---|---|
| ENT-001 | [M] ✅ | מבנה **רב-ישותי** (חברות/מטבעות) מהיום, גם אם ישות אחת בפועל (ARCH-008). **נבנה:** מודל `LegalEntity` + `entityId` על `JournalEntry`, `entities.ts`, מסך `/admin/entities`. **תנועות יומן ידניות מיוחסות לישות** (`postManualJournalEntry`/`postJournalEntry` מקבלים `entityId`, ברירת מחדל = ישות הבסיס דרך `getBaseEntityId`); פעילות אוטומטית (מכירה/PO) עדיין מתגלגלת לבסיס. |
