...
| מזהה | חיוב | דרישה |
|---|---|---|
| ENT-001 | [M] ✅ | מבנה **רב-ישותי** (חברות/מטבעות) מהיום, גם אם ישות אחת בפועל (ARCH-008). **נבנה:** מודל `LegalEntity` + `entityId` על `JournalEntry` (פעילות היסטורית מיוחסת לישות הבסיס), `entities.ts`, מסך `/admin/entities`. |
