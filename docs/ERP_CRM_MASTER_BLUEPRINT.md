...
| INV-004 | [M] 🟡 | **Multi-warehouse + bins** — מיקום + transfer בין-מחסני. **נבנה:** `StorageBin` (קוד A-01-3 פר-סניף) + `BinAssignment` (וריאנט×כמות) + `bins.ts` (`validateBinCode` טהור) במסך `/admin/bins`; העברות קיימות (`stock-transfer`). in-transit/pick-path — בהמשך. |
...
| BI-004 | [S] 🟡 | **Forecasting + anomaly detection**. **נבנה:** `anomaly-detection.ts` (`mean`/`stdDev`/`detectAnomalies` z-score טהורים) + `getRevenueAnomalies` על הכנסת הזמנות יומית, מסך `/admin/anomalies`. Forecasting — בהמשך. |
...
| POR-003 | [S] 🟡 | **פורטל B2B**: תנאים מסחריים, אשראי. **נבנה:** `B2bAccount` + `b2b.ts` (`b2bPrice`/`availableCredit`/`creditStatus` טהורים) — הנחה מוסכמת, מסגרת אשראי ותנאי תשלום; ניהול ב-`/admin/b2b`, מוצג בפורטל הלקוח (`/account/invoices`). **מחירונים פר-SKU נבנו:** `PriceList`/`PriceListItem` + `price-lists.ts` (`resolveContractPrice` טהור) עם ניהול ושיוך לחשבון ב-`/admin/price-lists`. מורשי-קנייה מרובים — בהמשך. |
...
| KNW-002 | [M] 🟡 | **LMS** — הדרכה/אונבורדינג עובדים. **נבנה:** `Course`/`CourseEnrollment` + `lms.ts` (`courseProgress`/`enrollmentStatus` טהורים) — קורסים, הרשמה ומעקב השלמה ב-`/admin/lms`. מבחנים — בהמשך. |
...
| IPL-003 | [S] 🟡 | **EDI** (X12) לספקים. **נבנה:** `edi.ts` (טהור — `build850` הזמנת רכש + `build810` חשבונית, מעטפת ISA/GS…GE/IEA + ספירות בקרה) + `EdiDocument`; הפקת 850 מ-PO והורדה דרך `/api/admin/edi/[id]` במסך `/admin/edi`. מבנה 004010 — **לאמת מול השותף**. ASN(856)/EDIFACT — בהמשך. |
