...
| FIN-TAX-003 | [M] 🟡 | **מבנה אחיד (קובץ SHAAM)** — ייצוא BKMVDATA/INI. **נבנה:** `shaam-export.ts` (טהור — A100/B100/Z900 + INI, מקודדי שדה padNum/padText/shaamDate/shaamAmount, סכומי בקרה) + `getShaamExportForPeriod` מה-GL, הורדה דרך `/api/admin/tax/shaam` ומסך `/admin/tax`. מבנה לפי הוראה 1.31 — **לאמת רוחב שדות + B110/C100/D110 מול רשות המסים/רו"ח**. |
| FIN-TAX-004 | [M] 🟡 | **ניכוי מס במקור** — חישוב וניכוי. **נבנה:** `WithholdingTaxRule` (שיעור לפי קטגוריה, מתוארך-תוקף) + `israeli-tax.ts` (`computeWithholding`/`effectiveWithholdingRate`/`getWithholdingRate`), מסך `/admin/tax`. שילוב ב-payment-run + דיווח 856 — בהמשך. שיעורים לדוגמה — **לאמת**. |
