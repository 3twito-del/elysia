...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| I-201 | Public UX and Brand | Actionable Now | P2 | S | `src/components/site-header.tsx`, `src/components/mobile-nav.tsx` | Header route labels | Audit route labels for duplicated Hebrew or stale English labels after navigation changes. | Header and mobile nav labels are unique, route-backed, and current. | Pending: source audit plus `pnpm test -- src/lib/public-structure-policy.test.ts`. |
...
| I-203 | Public UX and Brand | Actionable Now | P2 | S | `src/components/site-footer.tsx`, `scripts/qa-route-inventory.ts` | Footer grouping | Review footer legal, service, and commerce groups for stale routes or ambiguous labels. | Footer groups expose known routes with clear purpose. | Pending: `pnpm qa:routes` and footer source assertion. |
...
| I-222 | Commerce and Checkout | Actionable Now | P2 | S | `src/app/search/_lib/search-state.ts`, `src/app/search/page.tsx` | Search sort canonicalization | Extend search URL cleanup to canonicalize default sort and invalid sort values. | Shared search URLs are stable and reject unsupported sort values. | Pending: search state tests. |
| I-223 | Commerce and Checkout | Actionable Now | P2 | S | `src/app/category/[slug]/_lib/category-filter-state.ts` | Category filter reset cleanup | Verify reset links do not preserve stale tracking or empty filter values. | Reset returns to the category route with only meaningful parameters. | Pending: category filter tests. |
...
| I-226 | Commerce and Checkout | Actionable Now | P2 | S | `src/app/category/[slug]/_lib/category-filter-utils.ts`, `src/app/search/_lib/search-state.ts` | Price range bounds | Share price-bound normalization between category and search filters. | Invalid, negative, or reversed ranges normalize consistently. | Pending: category and search filter tests. |
...
| I-246 | Backend, API, and Data | Actionable Now | P1 | M | `src/server/http/safe-json.ts`, `src/server/http/api-response.ts` | Safe JSON edge cases | Extend safe JSON coverage for Date, BigInt, undefined, and nested unknown values. | Responses serialize predictably without leaking internals. | Pending: safe JSON tests. |
...
| I-258 | Backend, API, and Data | Actionable Now | P1 | S | `src/server/api/rate-limit.ts`, `src/server/services/rate-limit.ts` | Rate-limit namespace collisions | Add tests that protected endpoint namespaces cannot accidentally share counters. | Rate limits stay scoped by operation and identity. | Pending: rate-limit tests. |
...
| I-263 | Performance, PWA, and Reliability | Actionable Now | P1 | S | `src/app/sw.ts`, `src/app/serwist/[path]/route.ts` | Service worker route drift | Compare service-worker route handling against current route inventory. | Removed routes are not precached and critical routes remain covered. | Pending: serwist route tests. |
...
| I-265 | Performance, PWA, and Reliability | Actionable Now | P2 | S | `src/app/manifest.ts`, `public/*` | Manifest icon sizes | Audit manifest icons for required sizes, maskable purpose, and availability. | Install surfaces have complete icons without broken references. | Pending: manifest tests. |
...
| I-267 | Performance, PWA, and Reliability | Actionable Now | P2 | S | `src/app/layout.tsx`, `next.config.js` | Preconnect whitelist | Audit external preconnects and font or media origins. | Only current, useful origins are preconnected. | Pending: source guardrail. |
...
| I-272 | Performance, PWA, and Reliability | Actionable Now | P2 | S | `scripts/convert-public-images-to-avif.mjs`, `public` | AVIF repair dry-run diff | Add dry-run output listing stale images and expected savings. | Image repair commands are reviewable before modifying assets. | Pending: AVIF script tests. |
| I-273 | Accessibility, Privacy, and Security | Actionable Now | P1 | S | `src/app/layout.tsx`, `src/lib/accessibility-guardrails.test.ts` | Skip link keyboard smoke | Verify skip link target remains first keyboard recovery path. | Keyboard users can reach main content quickly on every shell. | Pending: accessibility guardrail test. |
| I-274 | Accessibility, Privacy, and Security | Actionable Now | P2 | S | `src/styles/globals.css`, `src/components/ui/button.tsx` | Focus-visible token inventory | Audit focus rings across buttons, links, inputs, dialogs, and sheets. | Interactive elements have visible focus without color-only cues. | Pending: CSS source test. |
...
| I-277 | Accessibility, Privacy, and Security | Actionable Now | P1 | S | `src/components/cookie-consent-banner.tsx`, `src/lib/cookie-consent.ts` | Cookie audit redaction | Ensure consent audit events never include raw IP, user agent, or free-form IDs. | Consent state remains auditable with minimal data. | Pending: cookie privacy tests. |
...
| I-280 | Accessibility, Privacy, and Security | Actionable Now | P2 | S | `src/app/privacy/page.tsx`, `src/app/terms/page.tsx`, `src/app/accessibility/page.tsx` | Legal print readability | Add print or narrow-width readability checks for legal pages. | Legal content stays readable without decorative chrome. | Pending: accessibility source test. |
...
| I-287 | QA, Release, and Observability | Actionable Now | P2 | S | `scripts/qa-route-inventory.ts`, `docs/qa` | Production smoke roster refresh | Refresh the smoke route roster and expected status per route. | Smoke coverage includes representative public, admin, API, and PWA routes. | Pending: `pnpm qa:routes`. |
| I-288 | QA, Release, and Observability | Actionable Now | P2 | S | `docs/qa/production-deployment-evidence-ledger.md` | Release evidence template | Add fields for branch, commit, deployment ID, alias, health result, and log window. | Every release ties commit to live alias and health check. | Pending: docs check. |
...
| I-291 | QA, Release, and Observability | Actionable Now | P1 | S | `docs/qa/production-deployment-evidence-ledger.md`, Vercel CLI workflow | Rollback playbook | Add a short rollback and promote decision tree for failed production deploys. | Operators know whether to rollback, promote, or redeploy. | Pending: docs review. |
...
| I-293 | QA, Release, and Observability | Actionable Now | P2 | S | `docs/ENGINEERING_CONVENTIONS.md`, `vitest.config.ts` | Flaky test policy | Clarify how to handle flaky tests without hiding real regressions. | Any retry or isolation decision has an owner, reason, and follow-up. | Pending: docs check. |
...
| I-295 | QA, Release, and Observability | Actionable Now | P2 | S | `scripts/site-copy.ts`, `docs/SITE_COPY_MAP.md` | Copy map orphan detection | Add a check for copy-map entries whose source path no longer exists. | Copy map fails when stale source paths remain after refactors. | Pending: site-copy tests. |
...
| I-298 | QA, Release, and Observability | Actionable Now | P2 | S | `vitest.config.ts`, `package.json` | Test wall-clock regression budget | Track expected duration for full check and flag large unexplained increases. | Runtime changes are visible during release preparation. | Pending: docs or script test. |
| I-299 | QA, Release, and Observability | Actionable Now | P2 | S | `docs/MULTI_ASPECT_IMPROVEMENT_BACKLOG.md`, `src/styles/qa-benchmark-traceability.test.ts` | Backlog rotation traceability | Update traceability rules when completed backlog batches rotate out of active work. | Historical IDs remain intentional and new active IDs are discoverable. | Pending: `pnpm test -- src/styles/qa-benchmark-traceability.test.ts`. |
| I-300 | QA, Release, and Observability | Actionable Now | P1 | M | Vercel CLI workflow, `docs/qa/production-deployment-evidence-ledger.md` | Post-deploy log scan automation | Add a repeatable command or script for post-deploy production error-log scans. | Release evidence records a clean log window or linked remediation. | Pending: Vercel inspect and logs workflow docs. |
