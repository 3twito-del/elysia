...
- Completed 2026-05-14: Audited image priority, responsive `sizes`, aspect ratios, and lazy-loading behavior across hero media, product cards, product galleries, and category/search media. Limited product-gallery `priority` to the initial active image, lazy-loaded later active gallery images, and expanded static image-performance guardrails for hero first-slide priority, product-card aspect/sizes, gallery main/thumb behavior, and fixed-size support media.
- Completed 2026-05-14: Prevented another hydration/date-drift slice by keeping admin coupon form validation free of client-clock `startsAt` defaults so the server schema owns coupon start time, and expanded hydration guardrails against client state wall-clock initializers, browser-only `typeof window` branching in client-rendered surfaces, missing explicit Hebrew commerce timezone formatting, and coupon forms reintroducing client-clock defaults.
- Completed 2026-05-14: Kept large commerce surfaces stable during hover, focus, loading, and refresh states by matching category loading product skeleton geometry to real product cards, preserving transform-only lift interactions for hover/focus, and adding layout-stability guardrails for product-card skeletons and shared motion selectors.
