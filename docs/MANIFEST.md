...
Hebrew-first (RTL) luxury-jewelry commerce platform plus a single-tenant
internal ERP/CRM suite, run as one system with one PostgreSQL source of truth.
Currently pre-revenue: no live payment processing and no real products yet.

The maximal internal ERP/CRM superset is a deliberate scope decision — an
internal-only capability build, not a business-need claim. Its breadth
expansion is currently paused by ADR 0001 (irreversibility before ambition);
the binding functional spec is `docs/ERP_CRM_MASTER_BLUEPRINT.md`.

## Document map

| Document                           | Category                                                                    |
| ---------------------------------- | --------------------------------------------------------------------------- |
| `docs/MANIFEST.md`                 | What the system is + the ubiquitous language                                |
| `docs/DESIGN.md`                   | Public change gate (blocking), design manifesto, standing visual invariants |
| `docs/ENGINEERING.md`              | Engineering conventions, benchmark method, release procedure                |
| `docs/DECISIONS.md`                | ADR register (0001–0014), append-only                                       |
