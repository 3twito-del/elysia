...
test.describe("authenticated customer fixture", () => {
  ...
  test("opens the account dashboard with reusable customer state", async ({
    ...
    const fixture = await signInCustomerWithFixture(page);

    await expect(page.getByTestId("account-local-order")).toBeVisible();
    await expect(
      page.getByTestId("account-shopify-mirror-order"),
    ).toBeVisible();
    await expect(
      page.getByTestId("account-wishlist-decision-support"),
    ).toBeVisible();
    await expect(page.getByTestId("account-saved-sizes-form")).toBeVisible();
    await expect(
      page.getByTestId("account-privacy-shortcut-context"),
    ).toBeVisible();
    ...
    expect(exportBody.customer?.shopifyOrderMirrors?.length).toBeGreaterThan(0);
